...
export function Scanner() {
  ...
  const onSubmit = async () => {
    const response = await postData('/storeProduct', { barcode, name, stock, expiration, StoreId: 3 });
    toast(response.result ? '등록 성공!' : '등록 실패.. 관리자에게 문의해주세요.', { icon: '🚀' });
    initScan()
  }

  const getProduct = async () => {
    const response = await postData<GetStoreProduct>('/getStoreProduct', { barcode, id: 3 }).then(res => {
